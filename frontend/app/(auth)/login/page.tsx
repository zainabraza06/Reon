...
import { Eye, EyeOff, Lock, Mail, Shield, Zap, Users, Paperclip } from "lucide-react";
...
export default function LoginPage() {
  ...
  const features = [
    { icon: Lock,      label: "End-to-end encryption",  desc: "Only you & your contacts can read messages" },
    { icon: Zap,       label: "Voice & video calls",     desc: "Crystal-clear HD calls, always encrypted" },
    { icon: Users,     label: "Group chats",             desc: "Secure group conversations up to 500 members" },
    { icon: Paperclip, label: "Secure file sharing",     desc: "Share files without exposing your data" },
  ];
    ...
    <div className="min-h-full flex flex-col md:flex-row">
      ...
      <div className="hidden md:flex md:w-[44%] auth-left-panel relative overflow-hidden flex-col justify-center p-14 text-white">
        ...
        <div className="relative z-10 max-w-sm">
          ...
          <h1 className="text-5xl font-black tracking-tight leading-none mb-3">Reon</h1>
          <p className="text-purple-300/80 text-[15px] leading-relaxed mb-10">
            The only messenger where{" "}
            <span className="text-cyan-300 font-semibold">privacy is not optional</span>.
          </p>
          ...
          <ul className="space-y-5">
            {features.map(({ icon: Icon, label, desc }) => (
              <li key={label} className="flex items-start gap-3.5">
                <div className="w-8 h-8 rounded-lg feature-icon-wrap flex items-center justify-center shrink-0 mt-0.5">
          ...
          {/* Trust badge */}
          <div className="mt-10 flex items-center gap-3 glass-panel rounded-xl px-4 py-3">
            <Shield size={16} className="text-cyan-400 shrink-0" />
            <p className="text-xs text-purple-200/75 leading-snug">
      ...
      <div className="flex-1 flex items-center justify-center p-6 bg-[#f8f7ff] dark:bg-[#08081a]">
        <div className="w-full max-w-sm">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-8">Sign in to continue to Reon</p>
