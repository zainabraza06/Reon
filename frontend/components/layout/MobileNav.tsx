...
export default function MobileNav() {
  ...
  const allItems = [
    ...tabs,
    { href: null as null, icon: LogOut, label: "Logout", action: handleLogout },
  ] as const;
      ...
      <div className="flex items-stretch h-14">
        {allItems.map((item) => {
          ...
          return (
            ...
            >
              {isActive && (
                <span className="absolute top-0 left-1/2 -translate-x-1/2 w-7 h-[3px] rounded-b-full btn-gradient" />
              )}
              <div className={`relative p-1 rounded-xl transition-all ${isActive ? "bg-violet-100 dark:bg-violet-500/15" : ""}`}>
                <item.icon
                  size={20}
