...
class _ChatListScreenState extends State<ChatListScreen> with SingleTickerProviderStateMixin {
    ...
    setState(() {
      ...
      if (idx >= 0) {
        _chats[idx] = ChatListItem(
          id: _chats[idx].id, fullName: _chats[idx].fullName,
          username: _chats[idx].username, profilePic: _chats[idx].profilePic,
  ...
  void _onNewGroupMsg(dynamic d) {
    final m = d as Map?; if (m == null) return;
    final gid = m['groupId'] as String?; if (gid == null) return;
    setState(() {
  ...
  void _onStatus(dynamic d) {
    final m = d as Map?; if (m == null) return;
    final uid = m['userId'] as String?; if (uid == null) return;
    final online = m['isOnline'] as bool? ?? false;
    setState(() {
      for (final c in _chats) { if (c.id == uid) c.isOnline = online; }
    });
  }
  ...
  void _onFriendRemoved(dynamic d) {
    ...
    if (removedId != null) setState(() => _chats.removeWhere((c) => c.id == removedId));
  }
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChats  = _q.isEmpty ? _chats  : _chats.where((c) => c.fullName.toLowerCase().contains(_q.toLowerCase())).toList();
    final filteredGroups = _q.isEmpty ? _groups : _groups.where((g) => g.name.toLowerCase().contains(_q.toLowerCase())).toList();
    ...
    return Scaffold(
      ...
      body: Column(children: [
        ...
        Container(color: isDark ? ReonColors.surfaceDark : Colors.white,
          ...
          child: Row(children: [
            ShaderMask(shaderCallback: (b) => kBrandGradient.createShader(b),
              child: Text('Reon', style: GoogleFonts.inter(fontSize: 22, fontWeight: FontWeight.w900, color: Colors.white))),
            const Spacer(),
            IconButton(icon: Icon(Icons.refresh_rounded, color: isDark ? ReonColors.textMuted : Colors.grey.shade500), onPressed: _load, tooltip: 'Refresh'),
          ])),
        // Search
        Container(color: isDark ? ReonColors.surfaceDark : Colors.white,
          padding: const EdgeInsets.fromLTRB(12, 0, 12, 10),
          child: TextField(controller: _search, onChanged: (v) => setState(() => _q = v),
            decoration: InputDecoration(hintText: 'Search conversations',
              ...
              prefixIcon: const Icon(Icons.search, size: 18, color: ReonColors.textMuted),
              filled: true, fillColor: isDark ? ReonColors.bgDark : const Color(0xFFF3F4F6),
              contentPadding: const EdgeInsets.symmetric(horizontal: 14, vertical: 11),
              border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none),
              enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none),
              focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: ReonColors.primary, width: 1.5))))),
        // Tabs
        Container(color: isDark ? ReonColors.surfaceDark : Colors.white,
          padding: const EdgeInsets.fromLTRB(12, 0, 12, 10),
          child: Container(height: 36, decoration: BoxDecoration(color: isDark ? ReonColors.bgDark : const Color(0xFFF3F4F6), borderRadius: BorderRadius.circular(10)),
              ...
              filteredChats.isEmpty
                ...
                : RefreshIndicator(onRefresh: _load, child: ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return _ChatTile(
                        name: c.fullName, subtitle: c.previewText,
                        avatar: c.profilePic, isOnline: c.isOnline,
                        time: _fmt(c.lastMessageAt), unread: c.unreadCount,
                        onTap: () => Navigator.of(context).push(MaterialPageRoute(
                          builder: (_) => ChatScreen(userId: c.id, userName: c.fullName, userAvatar: c.profilePic))));
                    })),
              // Groups
              filteredGroups.isEmpty
                ...
                : RefreshIndicator(onRefresh: _load, child: ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return _ChatTile(
                        name: g.name, subtitle: g.lastMessageContent ?? 'No messages yet',
                        avatar: g.avatar, isOnline: false,
                        time: _fmt(g.lastMessageAt), unread: 0,
                        onTap: () => Navigator.of(context).push(MaterialPageRoute(
...
class _ChatTile extends StatelessWidget {
  ...
  const _ChatTile({required this.name, required this.subtitle, this.avatar, required this.isOnline, required this.time, required this.unread, required this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(onTap: onTap, child: Container(
      ...
      child: Row(children: [
        ChatAvatar(name: name, imageUrl: avatar, size: 50, isOnline: isOnline),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Expanded(child: Text(name, style: GoogleFonts.inter(fontWeight: FontWeight.w600, fontSize: 14.5, color: isDark ? ReonColors.textDark : ReonColors.textLight), overflow: TextOverflow.ellipsis)),
            Text(time, style: GoogleFonts.inter(fontSize: 11.5, color: unread > 0 ? ReonColors.primary : ReonColors.textMuted)),
          ]),
          const SizedBox(height: 3),
          Row(children: [
            Expanded(child: Text(subtitle, style: GoogleFonts.inter(fontSize: 13, color: ReonColors.textMuted, fontWeight: unread > 0 ? FontWeight.w500 : FontWeight.w400), overflow: TextOverflow.ellipsis, maxLines: 1)),
            if (unread > 0) ...[const SizedBox(width: 8), Container(
              height: 20, constraints: const BoxConstraints(minWidth: 20), padding: const EdgeInsets.symmetric(horizontal: 5),
...
class _Empty extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Center(child: Text(message, textAlign: TextAlign.center, style: GoogleFonts.inter(color: ReonColors.textMuted, fontSize: 14, height: 1.6)));
}
