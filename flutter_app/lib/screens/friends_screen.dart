...
class _FriendsScreenState extends State<FriendsScreen>
  ...
  late final TabController _reqTab;
  ...
  void initState() {
    ...
    _reqTab = TabController(length: 2, vsync: this);
    _load();
    _listenSocket();
  }
        ...
        Expanded(
            child: _loading
                ...
                : TabBarView(controller: _tab, children: [
                    ...
                    // Requests tab — Received / Sent sub-tabs
                    Column(children: [
                      Container(
                        ...
                        child: TabBar(
                          ...
                          tabs: [
                            Tab(
                                child: Text(
                                    'Received${_received.isNotEmpty ? " (${_received.length})" : ""}')),
                            Tab(
                                child: Text(
                                    'Sent${_sent.isNotEmpty ? " (${_sent.length})" : ""}')),
                          ],
                        ),
                      ),
                      Expanded(
  ...
  void _showRemoveDialog(ReonUser f) {
    showDialog(
        ...
        builder: (ctx) => AlertDialog(
              title: Text('Remove ${f.fullName}?',
                  style: GoogleFonts.inter(fontWeight: FontWeight.w700)),
              content: Text(
                  'They won\'t be notified, but you\'ll need to send a new friend request to reconnect.',
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(
          ...
          Text(subtitle ?? user.displayName,
              style: GoogleFonts.inter(
                  fontSize: 12.5, color: ReonColors.textMuted)),
        ])),
      ...
      child: Container(
          ...
          decoration: BoxDecoration(
              ...
              boxShadow: gradient
                  ? [
                      BoxShadow(
                          color: ReonColors.primary.withValues(alpha: 0.25),
                          blurRadius: 8)
                    ]
                  : null),
          child: loading
...
class _Empty extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(
      child: Text(msg,
          style: GoogleFonts.inter(color: ReonColors.textMuted, fontSize: 14)));
}
