...
class _SignupScreenState extends State<SignupScreen> {
  ...
  Future<void> _submit() async {
    if (_name.text.trim().isEmpty || _email.text.trim().isEmpty || _password.text.length < 8) return;
    setState(() => _loading = true);
    final ok = await context.read<AuthProvider>().signup(_name.text.trim(), _email.text.trim(), _password.text);
      ...
      body: SafeArea(child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          Center(child: Container(width: 60, height: 60,
            decoration: BoxDecoration(gradient: kBrandGradient, borderRadius: BorderRadius.circular(17),
              boxShadow: [BoxShadow(color: ReonColors.primary.withValues(alpha: 0.35), blurRadius: 18, offset: const Offset(0, 5))]),
            child: Center(child: Text('R', style: GoogleFonts.inter(fontSize: 26, fontWeight: FontWeight.w900, color: Colors.white))))),
          const SizedBox(height: 28),
          Text('Create account', textAlign: TextAlign.center,
            style: GoogleFonts.inter(fontSize: 24, fontWeight: FontWeight.w800, color: isDark ? Colors.white : ReonColors.textLight)),
          const SizedBox(height: 4),
          Text('Join Reon for encrypted messaging', textAlign: TextAlign.center,
            style: GoogleFonts.inter(fontSize: 13, color: ReonColors.textMuted)),
          const SizedBox(height: 32),
          AuthField(controller: _name,     hint: 'Full name',        icon: Icons.person_outline_rounded),
          const SizedBox(height: 12),
          AuthField(controller: _email,    hint: 'Email',            icon: Icons.mail_outline_rounded,   type: TextInputType.emailAddress),
          const SizedBox(height: 12),
          AuthField(controller: _password, hint: 'Password (min 8)', icon: Icons.lock_outline_rounded, obscure: !_showPw,
          ...
          GradButton(label: 'Create Account', loading: _loading, onTap: _loading ? null : _submit),
...
class _StrengthBar extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final strength = length < 8 ? 1 : length < 12 ? 2 : 3;
    ...
    return Row(children: [
      ...
      Text(labels[strength - 1], style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: colors[strength - 1])),
    ]);
  }
}
