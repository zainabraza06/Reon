...
class SettingsLinkDeviceScreen extends StatefulWidget {
  ...
  @override
  State<SettingsLinkDeviceScreen> createState() =>
      _SettingsLinkDeviceScreenState();
}
...
class _SettingsLinkDeviceScreenState extends State<SettingsLinkDeviceScreen> {
  ...
  Future<void> _init() async {
    try {
      ...
      final sessionId =
          await ApiService.instance.createLinkSession(pair.publicKey);
  ...
  Future<void> _onClaimed(dynamic data) async {
    ...
    try {
      ...
      final aesKey =
          CryptoService.instance.deriveTransferAesKey(_ecdhPrivate!, theirPub);
      ...
      final enc =
          await CryptoService.instance.encryptForTransfer(privJwk, aesKey);
  ...
  Widget _buildBody() {
    return switch (_step) {
      'generating' || 'transferring' => const Center(
      ...
      'done' => Center(
          ...
          const SizedBox(height: 12),
          Text('Keys transferred!',
              style:
                  GoogleFonts.inter(fontWeight: FontWeight.w700, fontSize: 18)),
          const SizedBox(height: 8),
          Text('The new device can now decrypt your messages.',
              style: GoogleFonts.inter(color: ReonColors.textMuted),
              textAlign: TextAlign.center),
        ])),
      _ => Column(children: [
          const Icon(Icons.shield_outlined,
              color: ReonColors.primary, size: 40),
          const SizedBox(height: 12),
          Text('Scan with your new device',
              style:
          ...
          if (_linkUrl.isNotEmpty)
            Container(
              ...
              child: QrImageView(
                  data: _linkUrl, size: 220, backgroundColor: Colors.white),
            ),
          const SizedBox(height: 16),
          OutlinedButton.icon(
            onPressed: _copyLink,
          ...
          Text('Waiting for device to connect…',
              style:
                  GoogleFonts.inter(fontSize: 12, color: ReonColors.textMuted)),
        ]),
    };
  }
}
