...
class _GroupChatScreenState extends State<GroupChatScreen> {
  ...
  @override
  void dispose() {
    _input.dispose(); _scroll.dispose();
  ...
  Future<void> _loadMessages({String? before}) async {
    ...
    try {
      final raw = await ApiService.instance.getGroupMessages(widget.groupId, before: before);
  ...
  Future<void> _send() async {
    ...
    for (final mem in _group!.members) {
      try {
        ...
        if (jwkStr != null) memberKeys.add({'userId': mem.user.id, 'encryptedKey': await _encryptForMember(text, jwkStr)});
      } catch (_) {}
    }
    ...
    if (myPubJwk != null && memberKeys.isNotEmpty) {
      ...
      final enc = await CryptoService.instance.encryptText(text, memberKeys.first['_pubJwk'] as Map<String, dynamic>? ?? myPubJwk);
      ciphertext = enc.ciphertext;
    }

    final payload = jsonEncode({'ciphertext': ciphertext, 'contentType': 'text', 'memberKeys': memberKeys});
    final fd = FormData.fromMap({'data': payload});

    try {
      final sent = await ApiService.instance.sendGroupMessage(widget.groupId, fd);
  ...
  void _onNewMsg(dynamic d) async {
    ...
    final msg = GroupMessage.fromJson(Map<String, dynamic>.from(m['message'] as Map));
    ...
    SocketService.instance.emit('group-message-delivered', {'messageId': msg.id, 'groupId': widget.groupId});
    await ApiService.instance.markGroupRead(widget.groupId).catchError((_) {});
  }
  ...
  void _onDelivered(dynamic d) {
    ...
    if (mounted) setState(() {
      _messages = [for (final msg in _messages)
        msg.id == mid ? msg.copyWith(deliveredTo: List.generate(count + 1, (i) => {'userId': i == 0 ? (context.read<AuthProvider>().user?.id ?? '') : 'member_$i', 'at': DateTime.now().toIso8601String()})) : msg];
    });
    void _ignore(int x) {} _ignore(total);
  }

  void _onRead(dynamic d) {
    final m = d as Map?; if (m == null) return;
    if ((m['groupId'] as String?) != widget.groupId) return;
    final ids   = List<String>.from((m['messageIds'] as List? ?? []).map((e) => e as String));
    final by    = m['readBy'] as String? ?? '';
    final idSet = ids.toSet();
    if (mounted) setState(() {
      _messages = [for (final msg in _messages)
        idSet.contains(msg.id)
          ? msg.copyWith(readBy: [...msg.readBy, {'userId': by, 'at': DateTime.now().toIso8601String()}])
          : msg];
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: isDark ? ReonColors.surfaceDark : Colors.white, elevation: 0, titleSpacing: 0,
        ...
        title: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
            Text(widget.groupName, style: GoogleFonts.inter(fontSize: 15, fontWeight: FontWeight.w600, color: isDark ? Colors.white : ReonColors.textLight)),
            Text('${_group?.memberCount ?? 0} members', style: GoogleFonts.inter(fontSize: 12, color: ReonColors.textMuted)),
          ])),
        ])),
      body: Column(children: [
        if (_hasMore && !_loading)
          GestureDetector(onTap: () => _loadMessages(before: _messages.first.sentAt.toIso8601String()),
        ...
        Expanded(child: _loading && _messages.isEmpty
          ...
          : CustomPaint(painter: _DotGrid(isDark: isDark), child: ListView.builder(
              controller: _scroll, padding: const EdgeInsets.symmetric(vertical: 10),
              itemCount: _messages.length,
              itemBuilder: (_, i) {
                final msg    = _messages[i];
                final isMine = msg.sender.id == me.id;
                return _GroupBubble(message: msg, isMine: isMine, memberCount: memberCount, myId: me.id);
              }))),
        // Input
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Expanded(child: Container(
              ...
              child: TextField(controller: _input, maxLines: null,
                ...
                decoration: InputDecoration(hintText: 'Message ${widget.groupName}…', hintStyle: GoogleFonts.inter(fontSize: 14.5, color: ReonColors.textMuted),
...
class _GroupBubble extends StatelessWidget {
  ...
  final String myId;
  const _GroupBubble({required this.message, required this.isMine, required this.memberCount, required this.myId});
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      padding: EdgeInsets.only(top: 2, bottom: 2, left: isMine ? 64 : 12, right: isMine ? 12 : 64),
      child: Column(crossAxisAlignment: isMine ? CrossAxisAlignment.end : CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
        if (!isMine) Row(children: [
          ChatAvatar(name: senderName, imageUrl: message.sender.profilePic, size: 20),
          const SizedBox(width: 6),
          Text(senderName, style: GoogleFonts.inter(fontSize: 11.5, fontWeight: FontWeight.w600, color: ReonColors.primary)),
        ]),
        const SizedBox(height: 2),
        Container(
          decoration: isMine
            ? gradientBubbleDecoration(roundBR: false)
            : BoxDecoration(color: isDark ? ReonColors.cardDark : Colors.white,
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.end, mainAxisSize: MainAxisSize.min, children: [
            ...
            Row(mainAxisSize: MainAxisSize.min, children: [
              Text(time, style: GoogleFonts.inter(fontSize: 11, color: isMine ? Colors.white.withValues(alpha: 0.6) : ReonColors.textMuted)),
              if (isMine) ...[const SizedBox(width: 3), _GroupTick(senderId: message.sender.id, readBy: message.readBy, deliveredTo: message.deliveredTo, memberCount: memberCount)],
            ]),
          ]),
        ),
      ]),
...
class _GroupTick extends StatelessWidget {
  ...
  final int memberCount;
  const _GroupTick({required this.senderId, required this.readBy, required this.deliveredTo, required this.memberCount});

  @override
  Widget build(BuildContext context) {
    final otherRead      = readBy.where((r) => r['userId'] != senderId).length;
    final otherDelivered = deliveredTo.where((d) => d['userId'] != senderId).length;
    final allRead        = memberCount > 0 && otherRead >= memberCount;

    if (allRead)           return Icon(Icons.done_all_rounded, size: 14, color: ReonColors.accent.withValues(alpha: 0.9));
    if (otherDelivered > 0) return Icon(Icons.done_all_rounded, size: 14, color: Colors.white.withValues(alpha: 0.6));
